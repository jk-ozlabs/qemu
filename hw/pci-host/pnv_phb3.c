...
/* This is called whenever the PHB LSI source ID register *or*
...
static void pnv_phb3_set_irq(void *opaque, int irq_num, int level)
{
    ...
    /* LSI only ... */
    if (irq_num > 3) {
        DBG_ERR(phb, "Unknown IRQ to set %d\n", irq_num);
    }
                                              ...
                                              bool is_write)
{
    ...
    case 00:
        /* DMA or 32-bit MSI ? */
        /* XXX Add check for 32-bit MSI enable bit in config reg */
        if ((addr & 0xffffffffffff0000ull) == 0xffff0000ull) {
